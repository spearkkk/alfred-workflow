...
def main(wf):
    ...
    if (end_mark):
        ...
        if sentence.strip():
            ...
            log.debug("source_to_destination: %s" % source_to_destination)
            ...
            if response:
                ...
                wf.add_item(title=u'Cannot translate, sentence: %s' % sentence, valid=False)
        ...
        wf.add_item(title=u'\' \" ; , . ? ! 으로 끝나야 번역을 합니다.', valid=False)
        wf.send_feedback()
