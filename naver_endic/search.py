...
def main(wf):
    ...
    for another in anothers:
        ...
        wf.add_item(title=u"%s:     %s" % (another_keyword, another_meaning),
                    ...
                    arg=another_keyword,
                    valid=True)
    searched_keyword = results['searchResultMap']['searchResultListMap']['WORD']['query']
    words = results['searchResultMap']['searchResultListMap']['WORD']['items'][0]['meansCollector'][0]['means']
    for word in words:
        meaning = word['value']
